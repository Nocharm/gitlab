...
# 주의: 라이브러리라 set -e 미사용 — 오류는 각 함수의 반환 코드로 전파한다.
...
registry_set() {             # key → {port, container, type} 기록
  ...
  if jq --arg k "$key" --argjson p "$port" --arg c "$container" --arg t "$type" \
      '.[$k] = {port:$p, container:$c, type:$t}' "$f" > "$tmp"; then
    mv "$tmp" "$f"
  else
    rm -f "$tmp"; return 1
  fi
...
registry_remove() {          # key 삭제
  local f="$1" key="$2"
  local tmp; tmp="$(mktemp)"
  if jq --arg k "$key" 'del(.[$k])' "$f" > "$tmp"; then
    mv "$tmp" "$f"
  else
    rm -f "$tmp"; return 1
  fi
...
port_in_use() {              # 호스트 docker가 해당 포트를 publish 중이면 0 (소켓 통해 호스트 데몬 조회)
  local port="$1"
  docker ps --format '{{.Ports}}' 2>/dev/null | grep -qF ":${port}->"
}
